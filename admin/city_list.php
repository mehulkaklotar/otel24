  ...
  <tr>
   ...
   <td width="1" bgcolor="#398FA8"><img src="images/spacer.gif" width="1" height="1" /></td>
          ...
          <tr>
            <td height="21" align="left" class="txt">
				<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="title">
                    <tr bgcolor="#EDEDED">
                      <td width="70%" height="25"><img src="images/heading_icon.gif" width="16" height="16" hspace="5">Manage City </td>
                      <td width="30%" align="right">
					   <select name="keycountry" class="txtfld txt" style="width:150px; " onchange="javascript:location.href='city_list.php?keycountry='+this.value;">
	...
	<?php } }?>
					  ...
					  &nbsp;&nbsp;<input name="b1" type="button" class="button" id="b1" value="Add City" onClick="location.href='city_addf.php'">
          ...
          <tr>
            <td height="21" align="left" class="txt">
				...
				<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="title">
                    ...
                    Search: <input type="text" id="txtsearch" name="txtsearch" value="<?php echo $_REQUEST['txtsearch'];?>" size="20" class="input" /> <input  type="submit" name="search" value="Search"  class="button" />&nbsp;<input type="button"  value="Show All"  onclick="javascript:location.href='city_list.php'" class="button" /><div id="container1" style="width:250px;"></div>
...
{
	$sql=executeQuery("select city.*,state.name_English,country.country_English,city.state_id,city.country_id as ccid,country.country_id from city inner join state on city.state_id=state.id inner join country on country.country_id=city.country_id where 1=1 $where and (city_English like '%".$_REQUEST['txtsearch']."%' and city_Turkish like '%".$_REQUEST['txtsearch']."%') order by $order_by $order_by2 limit $start,$pagesize");
	
	$reccnt=mysql_num_rows(executeQuery("select city.*,state.name_English,country.country_English,city.state_id,city.country_id as ccid,country.country_id from city inner join state on city.state_id=state.id inner join country on country.country_id=city.country_id where 1=1 $where and (city_English like '%".$_REQUEST['txtsearch']."%' and city_Turkish like '%".$_REQUEST['txtsearch']."%')"));
}
else
{
$sql=executeQuery("select city.*,state.name_English,country.country_English,city.state_id,city.country_id as ccid,country.country_id from city inner join state on city.state_id=state.id inner join country on country.country_id=city.country_id where 1=1 $where order by $order_by $order_by2 limit $start,$pagesize");

$reccnt=mysql_num_rows(executeQuery("select city.*,state.name_English,country.country_English,city.state_id,city.country_id as ccid,country.country_id from city inner join state on city.state_id=state.id inner join country on country.country_id=city.country_id where 1=1 $where"));
}
?>
          <tr>
                ...
                <tr>
				  ...
				  <?php if($reccnt>0){?>
					...
					<TD width="10%" align="center"><strong class="bigWhite">S.No. </strong></TD>
					  <TD width="21%" align="center"><span class="bigWhite"><strong>City(English)</strong></span><strong> <?php echo sort_arrows('city_English')?></strong></TD>
					  <TD width="21%" align="center"><span class="bigWhite"><strong>City(Turkish)</strong></span><strong> <?php echo sort_arrows('city_Turkish')?></strong></TD>
					...
					<TD align="center" class="txt" ><?php echo $i?></TD> 
					 ...
					 <?php echo ucwords($line['city_English']);?>
					 &nbsp;</TD>
					 <TD align="center" class="txt" >
					 <?php echo ucwords($line['city_Turkish']);?>
					 &nbsp;</TD>
					 <?php 
					 ...
					 <td valign="middle" align="center"><a href="city_addf.php?id=<?php print $line['id']?>&country_id=<?php echo $country_id ?>&state_id=<?=$state_id?>" class="orangetxt">Edit</a></td>
...
<input type="submit" name="Submit" value="Activate" class="button" onclick="return del_prompt(this.form,this.value,'state_del.php')">
