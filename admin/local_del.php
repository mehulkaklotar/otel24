...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{
		$sql2="delete from tbl_zip where local_id in ($str_rest_refs)"; 
		executeUpdate($sql2);
		$sql="delete from local where id in ($str_rest_refs)"; 
		executeUpdate($sql);
	...
	{
		$sql="update local set status=1 where id in ($str_rest_refs)";
		executeUpdate($sql);
	...
	{
		$sql="update local set status=0 where id in ($str_rest_refs)";
...
header("Location: local_list.php?country_id=$country_id&state_id=$state_id");
exit();
?>