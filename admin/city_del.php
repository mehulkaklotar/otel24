...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{
		$sql2="delete from tbl_zip where city_id in ($str_rest_refs)"; 
		executeUpdate($sql2);
		$sql="delete from city where id in ($str_rest_refs)"; 
		executeUpdate($sql);
	...
	{
		$sql="update city set status=1 where id in ($str_rest_refs)";
		executeUpdate($sql);
	...
	{
		$sql="update city set status=0 where id in ($str_rest_refs)";
		executeUpdate($sql);
...
elseif($Submit=='RemoveFromHome')
	{
		 $sql="update city set at_home=0 where id in ($str_rest_refs)";
		executeUpdate($sql);
...
header("Location: city_list.php?country_id=$country_id&state_id=$state_id");
exit();
?>