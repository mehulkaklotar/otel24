...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{
		$sqlf=mysql_query("select * from tbl_subcategory where id in ($str_rest_refs)");
		...
		$sql="delete from tbl_subcategory where id in ($str_rest_refs)"; 
		executeUpdate($sql);
	...
	{
		$sql="update tbl_subcategory set status=1 where id in ($str_rest_refs)";
		executeUpdate($sql);
	...
	{
		$sql="update tbl_subcategory set status=0 where id in ($str_rest_refs)";
		executeUpdate($sql);
	...
	$cccid=$_REQUEST['cid'];
header("Location: subcategory_list.php?cid=$cccid");
exit();?>