/** 
* Copyright 2005-2007 massimocorner.com
* @author      Massimo Foti (massimo@massimocorner.com)
* @version     1.3.2, 2007-07-06
 */
...
function tmt_textValidatorFactory(fieldNode){
	...
	if(fieldNode.getAttribute("tmt:required")){
		required = eval(fieldNode.getAttribute("tmt:required"));
	}
...
tmt_globalPatterns.filepath_jpg_gif_png = new RegExp("\\\\[\\w_]*\\.([gG][iI][fF])|([jJ][pP][eE]?[gG])|([pP][nN][gG])$");
...
function del_prompt(frmobj,comb,destUrl)
{
	var found=0;
	if(frmobj.elements['ids[]'].length>1){
		for(i=0;i<frmobj.elements['ids[]'].length;i++)
		{
			if(frmobj.elements['ids[]'][i].checked==true)
			found=1;
		}
	}
	else
	{
		if(frmobj.elements['ids[]'].checked==true)
		found=1;
	}
	if(found==0)
	{
		alert("Please select the record(s) to perform required Action.");
		return false;
	}
	if(comb=='Delete'){
...
tmt_globalRules.noSpaceValidation = function(fieldNode){
	spArr = fieldNode.value.split(" ");
	if(spArr.length > 1) {
		return false
	} else {
		return true;
...
tmt_globalRules.greaterthan = function(fieldNode) {
	var date1 = document.getElementById(fieldNode.getAttribute("tmt:greaterthan")).value;
	date1Arr = date1.split("/");
	date2Arr = fieldNode.value.split("/");
	var datevar1 = new Date(date1Arr[2],date1Arr[0],date1Arr[1]);
  	var datevar2 = new Date(date2Arr[2],date2Arr[0],date2Arr[1]);
...
tmt_globalRules.greaterthancurrentdate = function(fieldNode) {
	date2Arr = fieldNode.value.split("/");
	var datevar2 = new Date(date2Arr[2],date2Arr[0]-1,date2Arr[1]);
	dateobj = new Date();
	var datevar1 = new Date(dateobj.getYear(),dateobj.getMonth(),dateobj.getDate());
